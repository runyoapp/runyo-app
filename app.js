...
const t=todayStr();
content.innerHTML=`<div class="modal-title">Dag kiezen</div> <div style="display:flex;flex-direction:column;gap:6px;margin-top:8px"> ${dates.map(d=>{ const pd=parseDate(d); const isT=d===t; return `<button onclick="closeDayModal();openAddActivity('${d}')" style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;background:${isT?'rgba(198,242,78,0.08)':'var(--surface)'};border:1px solid ${isT?'var(--accent)':'var(--border)'};cursor:pointer;font-family:var(--font-d);font-weight:700;font-size:15px;color:${isT?'var(--accent)':'var(--text)'};text-align:left;-webkit-tap-highlight-color:transparent">
...
html=`<div style="position:relative"> <button id="avatarBtn" onclick="toggleAvatarMenu()" style="width:32px;height:32px;border-radius:50%;background:var(--accent);color:#000;border:none;font-family:var(--font-d);font-weight:800;font-size:15px;cursor:pointer;display:flex;align-items:center;justify-content:center">${initials}</button> <div id="avatarMenu" style="display:none;position:absolute;right:0;top:40px;background:var(--surface);border:1px solid var(--border);border-radius:8px;min-width:190px;z-index:500;box-shadow:0 8px 24px rgba(0,0,0,0.45);overflow:hidden"> <div style="padding:12px 14px;border-bottom:1px solid var(--border)"> <div style="font-family:var(--font-m);font-size:11px;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(email)}</div> </div> <button onclick="toggleAvatarMenu();switchTab('settings')" style="width:100%;background:none;border:none;padding:11px 14px;text-align:left;font-family:var(--font-m);font-size:11px;color:var(--text);cursor:pointer;display:flex;align-items:center;gap:8px" onmouseover="this.style.background='rgba(255,255,255,0.04)'" onmouseout="this.style.background='none'"> <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg> Instellingen </button> <button onclick="toggleAvatarMenu();authSignOut()" style="width:100%;background:none;border:none;border-top:1px solid var(--border);padding:11px 14px;text-align:left;font-family:var(--font-m);font-size:11px;color:var(--race-text);cursor:pointer;display:flex;align-items:center;gap:8px" onmouseover="this.style.background='rgba(244,67,54,0.06)'" onmouseout="this.style.background='none'"> <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg> Uitloggen </button> </div> </div>`;
}else{
html=`<button onclick="oauthConnectFlow()" style="background:var(--accent);color:#000;border:none;padding:7px 14px;font-family:var(--font-m);font-size:10px;font-weight:700;letter-spacing:0.5px;border-radius:6px;cursor:pointer">Inloggen</button>`;
}
...
h+=`<div class="rb-item" onclick="openDayFromRacesBar('${r.datum}')" style="cursor:pointer;min-height:80px;display:flex;flex-direction:column;${isMain?'border-left:2px solid var(--accent);padding-left:12px;':''}"> ${isMain?`<div style="font-family:var(--font-m);font-size:8px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--accent);margin-bottom:2px">★ DOEL</div>`:''} <div style="display:flex;align-items:flex-start;gap:4px;margin-bottom:2px"> <div style="margin-top:1px;flex-shrink:0">${RXIcon(iconKey,12,'var(--race-text)','var(--race-text)')}</div> <div class="rb-title" style="font-size:${titleSize}">${esc(r.titel||r.datum)}</div> </div> ${distStr?`<div class="rb-meta">${esc(distStr)}</div>`:''} ${goalStr?`<div class="rb-goal">${esc(goalStr)}</div>`:''} <div style="flex:1"></div> <div class="rb-countdown">${cd.val}<span>${cd.unit}</span></div> </div>`;
});
...
let h=`<div class="page-title" id="todayPageTitle" style="touch-action:pan-y"> <div> <div class="pt-kicker">${kicker}</div> <div class="pt-h">${off===0?'Vandaag':days[dayIdx(d)]+' '+d.getDate()+' '+mf[d.getMonth()]}</div> </div> <div style="display:flex;align-items:center;gap:6px"> <div style="display:flex;align-items:center;gap:6px"> ${off!==0?`<button onclick="state.dayOffset=0;renderToday()" style="background:var(--surface);border:1px solid var(--accent);padding:5px 10px;color:var(--accent);cursor:pointer;font-family:var(--font-m);font-size:9px;letter-spacing:1px;border-radius:999px;white-space:nowrap">← Vandaag</button>`:’’}
...
h+=`<div class="card"> <div class="rest-card-inner"> <div class="rest-emoji">😴</div> <div><div class="rest-title">${T('rest_day')}</div><div class="rest-sub">${T('rest_msg')}</div></div> </div> </div>`;
}else{
...
h+=`<div class="card ${border}" onclick="openDayModalRow(${row.rowIndex},'${t}')" style="padding:16px 16px 14px;cursor:pointer;-webkit-tap-highlight-color:transparent;margin-bottom:8px"> <div style="display:flex;justify-content:space-between;align-items:flex-start"> <div style="font-family:var(--font-m);font-size:10px;letter-spacing:1.5px;text-transform:uppercase;font-weight:600;color:${ti.text}">${T(ti.i18n)}${row.titel?' · '+esc(row.titel):''}</div> <div style="width:28px;height:28px;background:var(--bg);border:1px solid var(--border);display:flex;align-items:center;justify-content:center">${RXIcon(row.type?.split(',')[0].trim()||'run',16,'var(--text)','var(--accent)')}</div> </div>`;
if(row.km&&isRun){
...
if(tmr){
h+=`<div class="card" style="padding:14px 16px"> <div style="font-family:var(--font-m);font-size:9px;color:var(--muted);letter-spacing:1.5px;text-transform:uppercase;font-weight:600;margin-bottom:6px">${T('tomorrow')} · ${days[dayIdx(tmrDate)]} ${tmrDate.getDate()}</div> <div style="display:flex;align-items:center;gap:10px"> <div style="width:28px;height:28px;background:var(--bg);border:1px solid var(--border);display:flex;align-items:center;justify-content:center">${RXIcon(tmr.type?.split(',')[0].trim()||'rust',14,'var(--muted)','var(--accent)')}</div> <div style="font-family:var(--font-d);font-weight:700;font-size:16px;flex:1">${esc(tmr.titel||'')}</div> ${tmr.km?`<div style="font-family:var(--font-m);font-size:10px;color:var(--accent)">${esc(tmr.km)} km</div>`:''} </div> </div>`;
}
...
const loggedIn=typeof authGetToken===‘function’&&authGetToken()&&!authIsExpired();
if(loggedIn){
return `<div class="no-connection-hint" onclick="switchTab('settings')"> <div class="nch-icon">📋</div> <div><div class="nch-text">${T('setup_title')}</div><div class="nch-link">${T('setup_body')}</div></div> </div>`;
}
...
return `<div class="prev-feedback"> <div class="prev-feedback-header"> <span class="prev-feedback-label">${T('feedback_logged')}</span> <button class="edit-link" onclick="state.editingFeedback=true;renderToday()">${T('feedback_edit')}</button> </div> <div class="prev-feedback-text">${esc(existing)}</div> </div>`;
}
const stars=[‘😵’,‘😓’,‘😐’,‘💪’,‘🔥’].map((e,i)=>
`<button class="star-btn${rating>0&&(i+1)<=rating?' active':''}" data-val="${i+1}">${e}</button>`
).join(’’);
return `<div class="feedback-section"> <div class="feedback-title">${T('feedback_q')}</div> <div class="feedback-stars">${stars}</div> <textarea class="feedback-textarea" id="feedbackText">${esc(text)}</textarea> <button class="btn-primary" id="submitBtn" onclick="handleFeedbackSubmit('${esc(datum)}')">${isEdit?T('feedback_update'):T('feedback_save')}</button> ${isEdit?`<button class="btn-secondary" onclick="state.editingFeedback=false;renderToday()">${T(‘feedback_cancel’)}</button>`:’’}
...
h+=`<div data-upcoming-date="${date}" style="background:var(--surface);border:1px solid ${isTdy?'var(--accent)':'var(--border)'};padding:12px;margin-bottom:6px;opacity:${isPastDay&&!isTdy?0.45:1};transition:border-color 0.2s"> <div style="display:flex;align-items:center;gap:12px;margin-bottom:${activeRows.length>1?'10':'0'}px"> <div style="text-align:center;min-width:36px;padding-right:10px;border-right:1px solid var(--border);flex-shrink:0"> <div style="font-family:var(--font-m);font-size:9px;color:var(--muted)">${days[dayIdx(d)]}</div> <div style="font-family:var(--font-d);font-weight:800;font-size:18px;color:${isTdy?'var(--accent)':'var(--text)'}">${d.getDate()}</div> </div> <div style="flex:1;min-width:0"> ${activeRows.map((row,i)=>{const ti=typeOf(row.type);return`<div onclick="openDayModalRow(${row.rowIndex},'${date}')" style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:${i>0?'8px 0 0':0};${i>0?'border-top:1px solid var(--border);margin-top:8px':''}">
...
h+=`<div> <div class="plan-row${isTdy?' is-today':''}${isPast?' is-past':''}" onclick="openDayModal('${date}')"> <div class="plan-row-date"><strong>${days[dayIdx(d)]} ${d.getDate()}</strong>${MONTHS_NL[d.getMonth()]}</div> <div class="plan-row-emoji">·</div> <div class="plan-row-body"><div class="plan-row-title" style="color:var(--faint)">—</div></div> <div class="plan-row-km"></div> </div> </div>`;
});
h+=’</div>’;
...
h+=`<div class="fase-next-nudge" onclick="selectFaseByName('${esc(nextFase)}')"> <span style="color:var(--faint);margin-right:4px">${esc(T('next_fase'))}:</span> ${esc(nextFase)} <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M5 2l5 5-5 5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg> </div>`;
}
}
...
h+=`<div class="card ${rb}" onclick="${clickHandler}" data-date="${dateStr}" style="padding:14px 16px;margin-bottom:${idx<rows.length-1?'8':'10'}px;cursor:pointer;-webkit-tap-highlight-color:transparent"> <div style="display:flex;align-items:center;gap:10px;margin-bottom:${r.detail?'10':'0'}px"> <div style="width:32px;height:32px;background:var(--bg);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;flex-shrink:0"> ${RXIcon(iconKey,18,isRaceRow?'var(--race-text)':'var(--text)',isRaceRow?'var(--race-text)':'var(--accent)')} </div> <div style="flex:1;min-width:0"> <div style="font-family:var(--font-m);font-size:9px;letter-spacing:1.5px;text-transform:uppercase;font-weight:600;color:${rti.text};margin-bottom:2px">${T(rti.i18n)}${parsedRaceType?' · '+esc(parsedRaceType):''}</div> <div style="font-family:var(--font-d);font-weight:800;font-size:20px;line-height:1">${esc(r.titel||'Training')}</div> </div> ${r.km?`<div style="font-family:var(--font-d);font-weight:800;font-size:22px;color:${isRaceRow?'var(--race-text)':'var(--accent)'};flex-shrink:0">${esc(r.km)}<span style="font-size:12px;color:var(--muted)">km</span></div>`:''} </div> ${r.detail?`<div style="font-family:var(--font-m);font-size:11px;color:var(--muted);line-height:1.6;padding-top:10px;border-top:1px solid var(--border)">${esc(r.detail)}</div>`:''} </div>`;
});
...
if(isEdit){const m=existing.match(/^(\d)/);if(m)rating=parseInt(m[1]);const mt=existing.match(/–\s*(.+)$/);if(mt)text=mt[1];}
if(isEdit&&!state.editingFeedback){
return `<div class="prev-feedback"> <div class="prev-feedback-header"> <span class="prev-feedback-label">${T('feedback_logged')}</span> <button class="edit-link" onclick="state.editingFeedback=true;openDayModal('${datum}')">${T('feedback_edit')}</button> </div> <div class="prev-feedback-text">${esc(existing)}</div> </div>`;
}
const stars=[‘😵’,‘😓’,‘😐’,‘💪’,‘🔥’].map((e,i)=>
`<button class="star-btn${rating>0&&(i+1)<=rating?' active':''}" data-val="${i+1}">${e}</button>`
).join(’’);
return `<div class="feedback-section"> <div class="feedback-title">${T('feedback_q')}</div> <div class="feedback-stars">${stars}</div> <textarea class="feedback-textarea" id="modalFbText">${esc(text)}</textarea> <button class="btn-primary" id="modalSubmitBtn" onclick="handleModalFeedback('${esc(datum)}')">${isEdit?T('feedback_update'):T('feedback_save')}</button> ${isEdit?`<button class="btn-secondary" onclick="state.editingFeedback=false;openDayModal('${datum}')">${T(‘feedback_cancel’)}</button>`:’’}
...
h+=`<div style="background:${r.type==='race'?'rgba(244,67,54,0.06)':'rgba(255,255,255,0.02)'};border:1px solid ${r.type==='race'?'rgba(244,67,54,0.3)':'var(--border)'};padding:12px 14px;margin-bottom:8px;display:flex;align-items:center;gap:10px" onclick="openDayModal('${state.calSelectedDate}')"> <div style="width:28px;height:28px;display:flex;align-items:center;justify-content:center">${RXIcon(r.type?.split(',')[0].trim()||'run',18,'var(--text)','var(--accent)')}</div> <div style="flex:1;min-width:0"> <div style="font-family:var(--font-m);font-size:9px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:${ti.text};margin-bottom:2px">${T(ti.i18n)}</div> <div style="font-family:var(--font-d);font-weight:700;font-size:15px">${esc(r.titel||'')}</div> ${r.km?`<div style="font-family:var(--font-m);font-size:10px;color:var(--accent);margin-top:2px">${esc(r.km)} km</div>`:''} </div> </div>`;
});
}else{
...
const cd=countdownDisplay(daysUntil(r.datum));
h+=`<div class="cal-race-row" onclick="openDayModal('${r.datum}')"> <div style="width:28px;height:28px;display:flex;align-items:center;justify-content:center">${RXIcon('race',20,'var(--race-text)','var(--race-text)')}</div> <div style="flex:1;min-width:0"> <div style="font-family:var(--font-m);font-size:9px;color:var(--race-text);letter-spacing:1px;text-transform:uppercase;font-weight:600;margin-bottom:2px">Race</div> <div style="font-size:13px;font-weight:600">${esc(r.titel||r.datum)}</div> ${r.km?`<div style="font-family:var(--font-m);font-size:10px;color:var(--muted)">${esc(r.km)} km</div>`:''} </div> <div style="text-align:right;flex-shrink:0"> <div style="font-family:var(--font-d);font-size:18px;color:var(--accent)">${cd.val}</div> <div style="font-family:var(--font-m);font-size:8px;color:var(--faint)">${cd.unit}</div> </div> </div>`;
});
}
h+=’</div>’;
...
state.*raceFromSheet=syntheticRace;
closeDayModal();
openRaceModal(’sheet*’+rowIndex);
}
...
content.innerHTML=` <div class="modal-title">${raceId?'Race bewerken':'Race toevoegen'}</div> <div class="settings-field"> <label class="settings-label">Race naam</label> <input type="text" class="settings-input" id="raceNameInput" value="${esc(race?.name||'')}" placeholder="Big10 Rotterdam"> </div> <div class="settings-field"> <label class="settings-label">Datum</label> <input type="date" class="settings-input" id="raceDateInput" value="${esc(race?.date||prefillDate||'')}"> </div> <div class="settings-field"> <label class="settings-label">Afstand</label> <select class="settings-input" id="raceDistSelect" onchange="document.getElementById('raceDistCustom').style.display=this.value==='__custom'?'block':'none'"> <option value="">—</option> ${DIST.map(d=>`<option value=”${d}”${matchedDist===d?’ selected’:’’}>${d}</option>`).join('')} <option value="__custom"${customDist?' selected':''}>Anders…</option> </select> <input type="text" class="settings-input" id="raceDistCustom" placeholder="bijv. 800 m" style="margin-top:6px;display:${customDist?'block':'none'}" value="${esc(customDist)}"> </div> <div class="settings-field"> <label class="settings-label">Type race</label> <select class="settings-input" id="raceTypeSelect" onchange="document.getElementById('raceTypeCustom').style.display=this.value==='__custom'?'block':'none'"> <option value="">—</option> ${TYPE.map(t=>`<option value=”${t}”${matchedType===t?’ selected’:’’}>${t}</option>`).join('')} <option value="__custom"${customType?' selected':''}>Anders…</option> </select> <input type="text" class="settings-input" id="raceTypeCustom" placeholder="bijv. Veldloop" style="margin-top:6px;display:${customType?'block':'none'}" value="${esc(customType)}"> </div> <div class="settings-field"> <label class="settings-label">Doeltijd (optioneel)</label> <input type="text" class="settings-input" id="raceGoalInput" value="${esc(race?.goal||'')}" placeholder="bijv. 37:30"> </div> <div class="settings-field"> <label class="settings-label">Notities / detail</label> <textarea class="settings-input" id="raceDetailInput" rows="3" style="resize:none">${esc(race?.notes||race?.detail||'')}</textarea> </div> <div class="settings-field" style="display:flex;align-items:center;gap:10px"> <input type="checkbox" id="raceMainInput" ${race?.mainGoal?'checked':''} style="width:18px;height:18px;accent-color:var(--accent)"> <label for="raceMainInput" class="settings-label" style="margin:0">Hoofddoel</label> </div> <button class="btn-primary" style="margin-top:8px" onclick="saveRace()">Race opslaan</button> ${race?`<button class="btn-secondary" onclick="deleteRace('${race.id}')">🗑 Verwijderen</button>`:''}`;
...
let h=`<div class="stats-grid"> <div class="stat-card"><div class="stat-label">${T('stats_total')}</div><div class="stat-val">${totalKm.toFixed(0)}</div><div class="stat-sub">${T('stats_done')}</div></div> <div class="stat-card"><div class="stat-label">${T('stats_days')}</div><div class="stat-val">${daysLeft}</div><div class="stat-sub">${esc(raceName)}</div></div> <div class="stat-card"><div class="stat-label">${T('stats_runs')}</div><div class="stat-val">${runCount}</div><div class="stat-sub">${T('stats_sessions')}</div></div> <div class="stat-card"><div class="stat-label">${T('stats_week')}</div><div class="stat-val">${weekKm.toFixed(0)}</div><div class="stat-sub">${T('stats_week_sub')}</div></div> ${avgRating>0?`<div class="stat-card"><div class="stat-label">${T(‘stats_feel’)}</div><div class="stat-val" style="font-size:28px">${[‘😵’,‘😓’,‘😐’,‘💪’,‘🔥’][Math.round(avgRating)-1]}</div><div class="stat-sub">${avgRating.toFixed(1)}/5 · ${ratingRows.length} ${T(‘stats_fb_sub’)}</div></div>`:’’}
...
recent.forEach(row=>{
const d=parseDate(row.datum);
h+=`<div class="fh-row"> <div class="fh-date">${d.getDate()} ${months[d.getMonth()]}</div> <div class="fh-body"><div class="fh-title">${esc(row.titel||row.type||'Training')}</div><div class="fh-text">${esc(row.feedback)}</div></div> ${row.km?`<div class="fh-km">${esc(row.km)}km</div>`:''} </div>`;
});
h+=’</div>’;
}
el.innerHTML=h;
...
el.innerHTML=`<div class="account-row"> <div class="account-avatar" style="font-size:22px">🏃</div> <div class="account-info"> <div class="account-email">${esc(oauthEmail)}</div> <div class="account-status">Google · ${T('logged_in_as')}</div> </div> <button class="account-logout" onclick="authSignOut()">${T('logout_btn')}</button> </div>`;
}else{
el.innerHTML=`<div style="font-size:12px;color:var(--muted);margin-bottom:12px"> Log in met Google om je schema te koppelen en data te synchroniseren. </div> <button class="btn-primary" onclick="oauthConnectFlow()">Inloggen met Google</button>`;
}
}
...
if(connected){
el.innerHTML=` <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px"> <div style="width:8px;height:8px;border-radius:50%;background:var(--run-text);flex-shrink:0"></div> <div style="flex:1;min-width:0"> <div style="font-size:12px;font-weight:600;margin-bottom:2px">${esc(state.sheetName||'Schema')}</div> <a href="https://docs.google.com/spreadsheets/d/${esc(sheetId)}/edit" target="_blank" style="font-family:var(--font-m);font-size:10px;color:var(--accent);text-decoration:none">Sheet openen ↗</a> </div> </div> <div id="sheetPickerInline"></div> <div style="display:flex;gap:8px;margin-top:8px"> <button class="btn-save" style="flex:1" onclick="loadSheetPickerInline()">Ander schema</button> <button class="disconnect-btn" style="flex:1" onclick="oauthDisconnect()">${T('connect_disconnect')}</button> </div> ${_devBlock()}`;
return;
}

if(oauthActive&&!sheetId){
el.innerHTML=` <div style="font-family:var(--font-m);font-size:11px;color:var(--muted);margin-bottom:12px">Ingelogd als <strong>${esc(authEmail())}</strong></div> <button class="btn-primary" style="margin-bottom:8px" onclick="oauthPickExisting()">Bestaand schema koppelen</button> <button class="btn-secondary" onclick="oauthCreateNew()">Nieuw leeg schema aanmaken</button> ${_devBlock()}`;
return;
}

el.innerHTML=` <div style="font-family:var(--font-m);font-size:11px;color:var(--muted);line-height:1.6;margin-bottom:14px">Koppel je Google Sheets trainingsschema.</div> <button class="btn-primary" id="oauthConnectBtn" onclick="oauthConnectFlow()" style="width:100%;margin-bottom:8px">Koppel met Google</button> ${_devBlock()}`;
}
...
el.innerHTML=`<div style="display:flex;flex-direction:column;gap:4px;margin-bottom:4px"> ${sheets.slice(0,5).map(s=>{ const d=new Date(s.modifiedTime); return `<button onclick="oauthSelectSheet('${s.id}','${esc(s.name)}')" style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:var(--bg);border:1px solid var(--border);border-radius:4px;cursor:pointer;text-align:left;width:100%">
...
</button>`; }).join('')} <div style="display:flex;gap:6px;margin-top:4px"> <input type="url" class="settings-input" id="inlineSheetUrl" placeholder="Of plak sheet URL…" style="flex:1"> <button class="btn-save" onclick="(function(){const u=document.getElementById('inlineSheetUrl')?.value.trim();const m=u?.match(/\/spreadsheets\/d\/([a-zA-Z0-9_-]+)/);if(!m){showToast('Ongeldige URL');return;}oauthSelectSheet(m[1],u);})()">OK</button> </div> </div>`;
...
c.innerHTML=active.map(d=>` <div class="pr-row"> <label class="pr-dist-lbl">${d}</label> <input type="text" class="settings-input" style="flex:1" placeholder="${T('pr_placeholder')}" value="${esc(prs[d]||'')}" oninput="updatePR('${d}',this.value)"> <button onclick="removePrField('${d}')" style="background:none;border:none;color:var(--faint);cursor:pointer;font-size:18px;padding:0 4px;line-height:1">×</button> </div>`).join(’’);
}
...
c.innerHTML=PR_ORDER.filter(d=>d in prs).map(d=>` <div class="pr-row" style="margin-bottom:8px"> <label class="pr-dist-lbl">${d}</label> <input type="text" class="settings-input" style="flex:1" placeholder="${T('pr_placeholder')}" value="${esc(prs[d]||'')}" oninput="updatePR('${d}',this.value)"> </div>`).join(’’);
}
