...
export function MetricPills({ theme, pace, hr, hasIntervals }: {
  theme: Theme
  pace?: string | null
  hr?: string | null
  hasIntervals?: boolean
}) {
  const pills = [pace, hr, hasIntervals ? 'intervallen' : null].filter(Boolean) as string[]
  if (pills.length === 0) return null
  return (
    <View style={styles.row}>
      {pills.map((p, i) => (
        <Text key={i} style={[styles.pill, { color: theme.muted, borderColor: theme.border }]}>{p}</Text>
      ))}
    </View>
  )
}
