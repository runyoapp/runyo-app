...
content.innerHTML=`<div class="modal-title">Schema koppelen</div> <div style="font-family:var(--font-m);font-size:11px;color:var(--muted);margin-bottom:16px">Ingelogd als <strong>${authEmail()}</strong></div> <button class="btn-primary" style="margin-bottom:10px" onclick="oauthPickExisting()">Bestaand schema koppelen</button> <button class="btn-secondary" onclick="oauthCreateNew()">Nieuw leeg schema aanmaken</button>`;
}
...
if(!sheets.length){
content.innerHTML+=`<div style="font-size:12px;color:var(--muted)">Geen Google Sheets gevonden. Maak een nieuw schema aan.</div> <button class="btn-primary" style="margin-top:12px" onclick="oauthCreateNew()">Nieuw schema aanmaken</button>`;
return;
}
const months=[‘jan’,‘feb’,‘mrt’,‘apr’,‘mei’,‘jun’,‘jul’,‘aug’,‘sep’,‘okt’,‘nov’,‘dec’];
content.innerHTML=`<div class="modal-title">Schema kiezen</div> <div style="font-family:var(--font-m);font-size:10px;color:var(--muted);letter-spacing:1px;text-transform:uppercase;margin-bottom:10px">Recente sheets</div> <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:12px"> ${sheets.map(s=>{ const d=new Date(s.modifiedTime); const ago=`${d.getDate()} ${months[d.getMonth()]}`; return `<button onclick="oauthSelectSheet('${s.id}','${esc(s.name)}')" style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;background:var(--surface);border:1px solid var(--border);cursor:pointer;text-align:left;-webkit-tap-highlight-color:transparent">
...
</button>`; }).join('')} </div> <div style="font-family:var(--font-m);font-size:10px;color:var(--muted);margin-bottom:6px">Of plak een Sheet URL:</div> <input type="url" class="settings-input" id="oauthSheetUrl" placeholder="https://docs.google.com/spreadsheets/d/…"> <button class="btn-secondary" style="margin-top:8px" onclick="oauthSelectFromUrl()">Koppelen via URL</button>`;
}catch(e){
...
content.innerHTML=`<div class="modal-title">${esc(name)}</div> <div style="font-size:12px;color:var(--muted);margin-bottom:12px">Ontbrekende kolommen: <strong>${result.missing.join(', ')}</strong></div> <button class="btn-primary" onclick="oauthFixAndSelect('${sheetId}','${esc(name)}')">Kolommen automatisch toevoegen</button> <button class="btn-secondary" style="margin-top:8px" onclick="oauthPickExisting()">Andere sheet kiezen</button>`;
}
}catch(e){
content.innerHTML=`<div class="modal-title">Fout</div><div style="color:var(--race-text)">${esc(e.message)}</div>`;
}
...
const _em=authEmail();
if(*em){
localStorage.setItem(’sheetId*’+_em,sheetId);
const _sn=state?.sheetName||localStorage.getItem(‘sheetName’)||’’;
if(*sn)localStorage.setItem(’sheetName*’+_em,_sn);
}
